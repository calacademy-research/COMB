...
testData <- testData %>% 
  ...
  mutate(Caples_Severity_Class = factor(Caples_Severity_Class, levels = c("High","Mod","Low","Unburned_In","Unburned_Out"))) #fix to be a leveled factor
```
...
fct1<-  levels(TPHdata$Caples_Severity_Class) #levels(fct_reorder(.f = TPHdata$plotID_veg, .x = TPHdata$Caples_Severity_Class))
...
fct1<- levels(shrubData$Caples_Severity_Class) #edited to pass same levels on ... seems to work
...
fct1<- levels(cwdData$Caples_Severity_Class) #old not working: fct1<- levels(fct_reorder(as.factor(cwdData$plotID_veg), cwdData$Caples_Severity_Class))
...
fct1<- levels(fct_reorder(as.factor(shrubCoverData$plotID_veg), as.numeric(shrubCoverData$Caples_Severity_Class)))
...
testData %>% 
  ...
  pivot_longer(-c(1:3), names_to = "Shrubs", values_to = "value") %>% 
  group_by(plotID_veg, Caples_Severity_Class, Shrubs) %>% 
  arrange(desc(Treatment_Interval)) %>% View()
