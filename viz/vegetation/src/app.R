...
# check to see if directories exist on the local drive; if not, make them
if (dir.exists(here("viz/vegetation/input/")) == FALSE) {
  dir.create(here("viz/vegetation/input/"))
}

# call drive sync to make sure that the local drive has all files that are present on the google drive
drive_sync(here("viz/vegetation/input/"))

# create a symlink to the needed input file to run this script ("Caples_PlotData.csv")
# this file is located on the Google Drive, so we need a symlink to this file in the input directory associated with this script
Caples_PlotData_filename <- "Caples_PlotData.csv"
link_create(path = here("vegetation", "data_ingest", "output", Caples_PlotData_filename), new_path = here("viz", "vegetation", "input", Caples_PlotData_filename))

# point to needed input file
testData <- read_csv(here("viz", "vegetation", "input", Caples_PlotData_filename))
