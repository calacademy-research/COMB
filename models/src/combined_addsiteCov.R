...
model {
  ...
  mu.bCover ~ dnorm(0, 1)
  tau.bCover ~ dgamma(0.1, 0.1)
  
  # ...for detection [pc data]
  p10 ~ dbeta(2, 50) # p10 = Pr(y = 1 | z = 0)
  p11 ~ dunif(0, 1) # p11 = Pr(y = 1 | z = 1)
  
  # ...for detection [ARU data]
  lam ~ dunif(0, 1000) # lambda: rate of target-species calls detected
  ome ~ dunif(0, 1000) # omega: rate of non-target detections
  
  # Parameters for observation model of y
  psi0 ~ dnorm(mu.psi0, tau.psi0)
  bCover ~ dnorm(mu.bCover, tau.bCover)
...
inits <- function() {
  list(
    mu = c(1, 0.6), sigma = c(1, 0.1), 
    z = zst,
    psi = runif(1), 
    p10 = runif(1, 0, 0.05), p11 = runif(1, 0.5, 0.8),
    lam = runif(1, 1, 2), ome = runif(1, 0, 0.4),
    g = gst
  )
}
