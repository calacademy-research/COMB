...
model {
  ...
  lam ~ dunif(0,10) # lambda: rate of target-species calls detected per file
  ome ~ dunif(0,25) # omega: rate of non-target detections per file
...
inits <- function() {
  list(
    ...
    psi = runif(1), p11 = runif(1, 0.2, 0.8),
    lam = runif(1, 1, 10), ome = runif(1, 0, 20), 
    g = gst
  )
}
...
monitored <- c("psi", "p11", "lam", "ome", "mu", "sigma", "T_pc_obs", "T_pc_sim")
...
pp.check(jagsResult, "y_aru_sum", "y_aru_Sim", main="Posterior Predictive Check\nSum of detections - ARU")
