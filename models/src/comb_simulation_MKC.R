...
gen_simulated_data <- function(
  ...
  out <- jags(data_gen_string, data = input_data, parameters.to.save = c("y.aru", "y.pc", "score"), n.iter= 1, n.chains = 1)
  sim_data <- out$mcmc[[1]]
...
gen_jags_model_text <- function(
  ...
  close_string <- "mean_psi <- mean(psi)
  NOcc <- sum(z[]) 
  PropOcc <- NOcc/nsites}"
