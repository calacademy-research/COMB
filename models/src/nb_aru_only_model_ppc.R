...
speciesCode <- "OSFL" # must match prefiltering of dataML_model.csv
year <- 2021
threshold <- -100
...
model {
  ...
  for (i in 1:nsites) { # Loop over sites
    lam[i] <- lambda[i]*z[i] + omega + 0.00001
    mu[i] <- lam[i]/(r + lam[i])
    ...
    for(j in 1:nsurveys.aru) {
      y.aru[i,j] ~ dnegbin(mu[i], r)  # n_Total samples processed
    #GOF
      LLobs0[i,j] <- logdensity.negbin(y.aru[i,j], mu[i], r)
      y_aru_Sim[i,j] ~ dnegbin(mu[i], r)
      LLsim0[i,j] <- logdensity.negbin(y_aru_Sim[i,j], mu[i], r)
    }
