...
model {
  ...
  # Likelihood part 1 & 2: PC and ARU detections
    # Point count occurence
  for (i in 1:nsites) { # Loop over sites
    for (t in 1:nyears) {
    
      logit(psi[t,i]) <- beta0 + beta1*cover[i] + beta2*burn[i] + beta3*lgTree[i] + beta4*t
      z[t,i] ~ dbern(psi[t,i]) # Latent occupancy states

    # Point count detection probability
   
    for(j in 1:nsurveys.pc) {
      y.ind[t,i,j] ~ dbern(p[t,i,j]) # Observed occ. data (if available)
      p[t,i,j] <- z[t,i]*p11 # Detection probability
    }
...
#    T_pc_sim0[i] <- (sqrt(y_pc_Sim[i]) - sqrt(p11*z[i]*n_v[i]))^2  # ...and for simulated data
 ...
 #   zSim[i] ~ dbern(psiSim[i])
    ...
    for (j in 1:nsurveys.aru) {
      y.aru[t,i,j] ~ dbern(p_aru[t,i,j])  # n_surveys.aru = Total files processed
      p_aru[t,i,j] <- z[t,i]*p_aru11 + p_aru01 # Detection probability with false positives
    }
    }  # end year loop
...
# site_covars <- read_csv("./models/input/wide1havars.csv") %>%
#   mutate(Point = avian_point) %>%
