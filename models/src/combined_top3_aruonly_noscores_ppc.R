 ...
 # tau[2] <- 1 / (sigma[2] * sigma[2])
  ...
  #  }
    ...
    for(j in 1:nsurveys.aru) {
      y.aru[i,j] ~ dpois(lam*z[i] + ome + 0.0001)  # Total samples processed
    }
...
monitored <- c("psi",  "lam", "ome",  "T_aru_obs", "Dobs", "Dsim")
...
## Quick check using zero-inflated poisson
library(pscl)
test_df <- cbind.data.frame(y_aru_sum, n_samp_per_site, y_pc_sum)
summary(m1 <- zeroinfl(y_aru_sum ~1, data = subset(test_df, n_samp_per_site ==24)))