...
round(rbind( "det"  = predict(m1, type = 'det', newdata = pred.df), 
...
cand.mods <- list( "p(c)fp(c)psi(c*t)" = occuFP(~Cntry, ~Cntry, ~1, ~Cntry*sYear, data = lynx.umf), 
...
# Coefficients on the probability (="real") scale 
pred.df <- data.frame(Method = c("1", "2"))
round(rbind( "det" = predict(m2, type = 'det', newdata = pred.df),
