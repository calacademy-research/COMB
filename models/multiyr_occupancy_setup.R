...
PCTC %>%
  ...
  filter(Year!=2017) %>%
  distinct() %>% group_by(Year, point_ID_fk) %>%
  tally() %>%
  spread(Year, n) %>% View() 
...
dates1 <- visits %>% dplyr::select(-rich) %>% filter(visit<5) %>%
  mutate(JDay = as.numeric(format(DateTime, "%j"))) %>%
  select(-DateTime) %>%
  melt(id.var=c("point_ID_fk", "year", "visit"), measure.var="JDay") %>% acast(point_ID_fk ~ visit ~ year)
...
times <- visits %>% dplyr::select(-rich) %>% filter(visit<5) %>%
  ...
  select(-DateTime) %>%
  melt(id.var=c("point_ID_fk", "year", "visit"), measure.var="Time") %>% acast(point_ID_fk ~ visit ~ year)
...
nvisit <- visits %>% filter(visit<5) %>% group_by(point_ID_fk, year) %>% summarise(nvisits=max(visit))
...
data=list(N=nspec, # number of species (integer) 
