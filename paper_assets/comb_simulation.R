...
gen_data_text <- function(include_aru = TRUE,
                          include_pc = TRUE,
                          include_scores = TRUE,
                          include_covar = TRUE,
...
"
data_mod_string <-
  paste(string_init, occ_lik, pc_lik, aru_lik, close_string, sep = "\n")
...
sim_input_data <- function(nsites = 100,
                           p11 = 0.1,
                           p_aru11 = 0.1,
                           p_aru01 = 0.025,
                           beta0 = 0,
  ...
  input_data <- list(
    ...
    nsurveys.scores = nsurveys.scores,
...
gen_simulated_data <- function(include_aru = TRUE,
                               include_pc = TRUE,
                               include_scores = TRUE,
                               include_covar = TRUE,
                               aru_scores_independent = TRUE,
  ...
  data_gen_string <- gen_data_text(
    ...
    aru_scores_independent = aru_scores_independent
  )
  ...
  sim_data <- out$mcmc[[1]]
  y.aru.sim <-
    matrix(sim_data[1:(nsurveys.aru * nsites)], nrow = nsites)
  y.pc.sim <-
    matrix(sim_data[(1 + (nsurveys.aru * nsites)):((nsurveys.aru * nsites) + (nsurveys.pc * nsites))], nrow = nsites)
  score.sim <-
    matrix(sim_data[(1 + (nsurveys.aru * nsites) + (nsurveys.pc * nsites)):length(sim_data)], nrow = nsites)
  ...
  if (return_inputs) {
    ...
    sim_data_out <- list(y.aru = y.aru.sim,
                         y.pc = y.pc.sim,
                         score = score.sim)
  }
  return(sim_data_out)
...
gen_jags_model_text <- function(include_aru_model = TRUE,
                                include_pc_model = TRUE,
                                include_scores_model = TRUE,
                                include_covar_model = TRUE,
                                aru_scores_independent_model = TRUE,
                                psi_prior = "dbeta(2,2)",
  ...
  if (sum(include_pc_model, include_aru_model, include_scores_model) == 0)
    stop("Must include at least one source of observed data")
  
  if (include_covar_model) {
    reg_priors <-
      paste0("beta0 ~ ", beta0_prior, "\n beta1 ~ ", beta1_prior)
    ...
    priors_string <-
      paste(priors_string, "psi ~ ", psi_prior, sep = "\n")
    occ_lik <- "
    # Likelihood for occupancy
  ...
  if (include_pc_model) {
    ...
    priors_string <-
      paste(priors_string, p11_prior_string, sep = "\n")
  } else {
    pc_lik <- ""
  }
  ...
  if (include_aru_model & include_scores_model) {
    ...
    priors_string <-
      paste(priors_string,
            p_aru11_prior_string,
            p_aru01_prior_string,
            scores_prior,
            sep = "\n")
...
gen_monitored_param_list <- function(include_aru_model = TRUE,
                                     include_pc_model = TRUE,
                                     include_scores_model = TRUE,
                                     include_covar_model = TRUE) {
  ...
  if (include_covar_model) {
    monitored_list <-
      append(monitored_list, c("beta0", "beta1", "mean_psi"))
  } else {
...
fit_model_to_sim_data <- function(jagsInputData = NULL,
...
run_n_simulation_and_fit <- function(condition_name = "",
                                     n_sims = 1,
                                     include_aru = TRUE,
                                     include_pc = TRUE,
                                     include_scores = TRUE,
  ...
  jags_model_fit_text <- gen_jags_model_text(
    ...
    aru_scores_independent_model = aru_scores_independent_model,
    psi_prior = psi_prior,
  ...
  for (i in 1:n_sims) {
    ...
    for (param_name in rownames(jagsResult$summary)) {
      sim_results_list[[param_name]][i,] <-
        jagsResult$summary[param_name,]
    }
    sim_result_dic[i] <- jagsResult$DIC
  }
  
  
  sim_input_params <- expand_param_list(
    list(
      include_aru = include_aru,
      include_pc = include_pc,
      include_scores = include_scores,
...
get_all_metric_df <-
  function(stacked_list, metric_name, param_list) {
    df <- data.frame(
    ...
    for (i in 1:length(stacked_list)) {
      for (parameter_name in param_list) {
        metric_vec <-
          stacked_list[[i]]$sim_results_list[[parameter_name]][[metric_name]]
        condition_name_vec <-
          rep(stacked_list[[i]]$condition_name, length(metric_vec))
        param_name_vec <- rep(parameter_name, length(metric_vec))
        true_value_scalar <-
          stacked_list[[i]]$sim_input_params[[parameter_name]]
        true_value <- rep(ifelse(is.null(true_value_scalar), NA, true_value_scalar),
                          length(metric_vec))
        df_temp <-
          data.frame(metric_vec,
                     condition_name_vec,
                     param_name_vec,
                     true_value)
        df <- rbind(df, df_temp)
