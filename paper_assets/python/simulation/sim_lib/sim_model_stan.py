...
class StanSimModel:
    ...
    def __init__(self, params: SimParams):
        ...
        # generate the stan model text for the actual model
        self.model_text = self.gen_stan_model_text()
        ...
        self.model = stan.build(program_code=self.model_text, data=sim_data)
    ...
    def sample(self):
        ...
        return self.model.sample(
            num_chains=self.params.nc,
            num_samples=self.params.ni,
        )

    def gen_stan_model_text(self):
        """
        Generate STAN model text based on the simulation model parameters
        """
        params = self.params
        ...
        if params.include_covar_model:
            ...
            occ_lik = """
            // Vectorized likelihood for point counts and ARU observations
            for (i in 1:nsites) {
    ...
    @staticmethod
    def load_from_sim_results(sim_results: SimResults) -> "StanSimModel":
        """
        ...
        return StanSimModel(sim_params)
