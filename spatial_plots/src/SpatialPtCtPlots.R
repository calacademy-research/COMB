...
library(RgoogleMaps)
comb_map <- GetMap.bbox(
  lonR = range(year_pt_counts$longitude, na.rm = TRUE),
...
convert_points <- LatLon2XY.centered(
  comb_map,
...
surf <- mba.surf(cbind(coords, hamfly_ptct),
...
surf <- mba.surf(cbind(coords, hamfly_maxlgt),
  no.X = x.res, no.Y = y.res,
  h = 5, m = 2, extend = FALSE
)$xyz.est
