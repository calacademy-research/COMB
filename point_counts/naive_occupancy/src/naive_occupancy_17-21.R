...
diff <- PClong %>%
  ...
  mutate(post_g_l_n = case_when(
    `max2021` > max1719 ~ "greater",
...
diff <- diff %>% filter(
  birdCode_fk != "GCCR", # Removed non-species
  birdCode_fk != "SPHY",
  birdCode_fk != "0"
)
