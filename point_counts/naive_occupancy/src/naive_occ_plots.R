...
diffplot <- function(datadiff, dw, ww, ttl, ttl_size, txt_size, txt_angle) {
  ggplot(
    data = datadiff,
    aes(
      x = reorder(birdName, -naive_occ), y = naive_occ, fill = year,
    ...
    geom_errorbar(
      data = datadiff, stat = "identity", width = .5,
    ...
    theme_bw() +
    theme(
      plot.title = element_text(size = ttl_size), axis.title = element_text(size = txt_size),
