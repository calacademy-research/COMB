...
# parameters --------------------------------------------------------------
speciesCode <- "WETA" # must match prefiltering of dataML_model.csv
year <- c(2018,2019,2020,2021)
threshold <- 0
aruVisitLimit <- 24*2*3 # only consider this many ARU visits per site (ordered)
...
model {
 ...
 # beta0[2] ~ dunif(-5, 5) # Occupancy intercept on prob. scale
  ...
  for (i in 1:nsites) { # Loop over sites
    for (t in 1:nyears){
      # logit(psi[t,i]) <- beta0[t] 
      # beta0[t] <- z[t-1,i]*persistance[t] + (1-z[t-1,i])*colonization[t]
      # persistance[t] <- ifelse(t > 2, 1, 0)*burn[i]*beta_bp + ifelse(t < 2, 1, 0)*beta0_preburn
      logit(psi[t,i]) <- beta0[t] + beta1*burn[i]*ifelse(t > 2, 1, 0) # add in ifelse(t > 2, 1, 0)*max(z[0,i],z[1,i])*beta3 +  ifelse(t > 2, 1, 0)*(1 - max(z[0,i],z[1,i])*beta4
...
jagsResult_WETA_multiyear <- jags(
  jagsData,
  inits,
  monitored,
  modelFile,
